...
public class PaperServiceImpl implements PaperService {
        ...
        try {
            ...
            for (int i = 0; i < vos.size(); i++) {
                ...
                if (topics.size() < number) {
                    ...
                    return AjaxResult.error("《" + vo.getCourse().getName() + "》的”" + vo.getStage().getName() +
                            "“阶段下符合难度为”" + vo.getLevel().getName() + "“的" + vo.getType().getName() + "题目数量不足！");
                }
    ...
    public AjaxResult downPaper(Paper paper) {
        ...
        try {
            des_path = URLDecoder.decode(ResourceUtils.getURL("classpath:").getPath(), "UTF-8")
                    + "resources/down-paper";
        } catch (Exception e) {
            e.printStackTrace();
            return AjaxResult.error("程序异常");
        }
